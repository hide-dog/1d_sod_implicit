...
function inner_ite(Qcn,Qcm,nx,dx,dt,RHS,Amatrix_plus,Amatrix_minus,beta_sigma,norm_ok) # 内部反復
    delta_Q = zeros(nx,3)
...
function main()
    ...
    # -- 出力--
    dir_name       = "FVS_lusgs__jl_test"           # 出力フォルダ名
    ...
    for t in 1:nstep
        ...
        for innner_t in 1:innner_step
            ...
            #println(Amatrix_plus[51,:,:])

            Qcm = inner_ite(Qcn,Qcm,nx,dx,dt,RHS,Amatrix_plus,Amatrix_minus,beta_sigma,norm_ok)
        end
